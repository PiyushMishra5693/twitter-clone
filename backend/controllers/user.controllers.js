...
export const getSuggestedUser= async (req,res)=>{
    ...
    try {
        ...
        const filteredUsers = users.filter((user)=> !userFollowdByMe.following.includes(user._id));
...
export const updateUser = async (req,res)=>{
    ...
    try {
        ...
        if(profileImg){
            ...
            const uploadResponce = await cloudinary.uploader.upload(profileImg);
        ...
        if(coverImg){
            ...
            const uploadResponce = await cloudinary.uploader.upload(coverImg);
        ...
        console.log("Updated user error:",error.message);
        res.status(500).json({message:error.message});
    }
}